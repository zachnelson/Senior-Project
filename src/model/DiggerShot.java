...
public class DiggerShot extends EnemyFigure {
    ...
    public DiggerShot(float x, float y, float width, float height) {
        super(x, y); // origin: upper-left corner
        super.health = 1;
        super.damage = 1;
        this.width = width;
        this.height = height;
        super.state = GameFigureState.DIGGER_SHOT_STATE_LAUNCHED;
        mainImage = null;
        ...
        try {
            mainImage = ImageIO.read(getClass().getResource("/model/assets/bullet.png"));
    ...
    public void update() {
        ...
        if (state == GameFigureState.DIGGER_SHOT_STATE_LAUNCHED)
        {
            updateState();
            updateLocation();
        }
    ...
    public void updateState() {
            ...
            if (health <= 0)
            {
	           ...
	           playMusic(PlayerBoom);
                ...
                super.damage = 0;
                state = GameFigureState.DIGGER_SHOT_STATE_EXPLODED;
                missleHit();
                missileTimer.start();
            }
