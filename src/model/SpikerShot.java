...
public class SpikerShot extends EnemyFigure {
    ...
    public SpikerShot(float x, float y, float width, float height, double angle2) {
        super(x, y); // origin: upper-left corner
        super.health = 1;
        super.damage = 5;
        this.width = width;
        this.height = height;
        super.state = GameFigureState.SPIKER_SHOT_STATE_LAUNCHED;
        mainImage = null;
        angle = angle2;
        ...
        try {
            mainImage = ImageIO.read(getClass().getResource("/model/assets/SpikerEnemySpike.png"));
        } catch (IOException ex) {
            JOptionPane.showMessageDialog(null, "Error: Cannot open SpikerEnemySpike.png");
            System.exit(-1);
        }
    ...
    public void update() {
        ...
        if (state == GameFigureState.SPIKER_SHOT_STATE_LAUNCHED)
        {
            updateState();
            updateLocation();
        }
    ...
    public void updateState() {
            ...
            if (health <= 0)
            {
	           ...
	           playMusic(PlayerBoom);
                ...
                super.damage = 0;
                state = GameFigureState.SPIKER_SHOT_STATE_EXPLODED;
                missleHit();
                missileTimer.start();
            }
